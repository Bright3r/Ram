...
inline void upsilon61_1() noexcept
{
	...
	writeGraphsToFileAdj("graphs/61/upsilon1.adj", graphs);
}


inline void upsilon61_2() noexcept
{
	auto upsilon1 = loadBulkAdj("graphs/61/upsilon1.adj");
	std::vector<EdgeColoredUndirectedGraph> ts = { make_T1(), make_T2() };
	...
	// Save graphs
	writeGraphsToFileAdj("graphs/61/upsilon2.adj", graphs);
}


inline void upsilon61_3() noexcept
{
	auto upsilon2 = loadBulkAdj("graphs/61/upsilon2.adj");
	std::vector<EdgeColoredUndirectedGraph> ts = { make_T1(), make_T2() };
	...
	for (const auto& g : upsilon2)
	{
		...
		// embeddable into a good k16 for at least 1 color
		...
		for (auto u : attaching_set)
		{
			...
			// Reject coloring
			if (num_embeddable_neighborhoods < 1)
			{
				is_embeddable = false;
				break;
			}
	...
	// Save Graphs
	writeGraphsToFileAdj("graphs/61/upsilon3.adj", graphs);
}
...
inline void upsilon61_4() noexcept
{
	auto t_perms = make_tperms();
	auto upsilon3 = loadBulkAdj("graphs/61/upsilon3.adj");
	...
	// Save to file
	writeGraphsToFileAdj("graphs/61/upsilon4.adj", graphs);
}


inline void upsilon61_5() noexcept
{
	auto t_perms = make_tperms();
	auto upsilon4 = loadBulkAdj("graphs/61/upsilon4.adj");

	// Cull colorings that are not embeddable in one color
	...
	std::printf("%zu graphs are embeddable in one color\n", embeddable.size());
	...
	// Save to File
	writeGraphsToFileAdj("graphs/61/upsilon5.adj", graphs);
}
